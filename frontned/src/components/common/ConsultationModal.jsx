...
import { FiX, FiCalendar, FiClock, FiUser, FiCheck, FiChevronLeft, FiChevronRight } from "react-icons/fi";
...
const ConsultationModal = ({ isOpen, onClose }) => {
  ...
  const handleNext = () => {
    ...
    if (step === 3 && formData.name && formData.email) setStep(4);
  };
            ...
            <div className="min-h-[300px]">
              {step === 1 && (
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="text-lg font-semibold text-white">{currentMonth} {today.getFullYear()}</h4>
                    <div className="flex gap-2">
                      <button className="p-1 hover:bg-slate-800 rounded text-slate-400"><FiChevronLeft /></button>
                      <button className="p-1 hover:bg-slate-800 rounded text-slate-400"><FiChevronRight /></button>
                    </div>
                  </div>
                  <div className="grid grid-cols-7 gap-2 text-center mb-2">
                    {['S','M','T','W','T','F','S'].map(d => (
                      <span key={d} className="text-xs font-medium text-slate-500">{d}</span>
                    ))}
                  </div>
              ...
              {step === 2 && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -20 }}
                >
                  <h4 className="text-lg font-semibold text-white mb-6">Select Time</h4>
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1">Project Notes</label>
                      <textarea
              ...
              {step === 4 && (
                ...
                >
                  ...
                  <p className="text-slate-400 max-w-xs mx-auto">
                    We have scheduled your consultation for <strong className="text-white">{currentMonth} {selectedDate}</strong> at <strong className="text-white">{selectedTime}</strong>.
                  </p>
                </motion.div>
              )}
            </div>
